...
describe('jetton lockup', () => {
    ...
    describe('maintain tests', () => {
        it('should maintain, send transaction', async () => {
            const msg = beginCell()
                .storeUint(0x18, 6)
                .storeAddress(user.address)
                .storeCoins(BigInt(1e8))
                .storeUint(0, 1 + 4 + 4 + 64 + 32 + 1 + 1)
                .endCell()
            const tx = await jettonLockup.sendMaintain(owner.getSender(), msg, 3)
        ...
        it('shouldn`t maintain, not owner', async () => {
            const msg = beginCell()
                .storeUint(0x18, 6)
                .storeAddress(user.address)
                .storeCoins(BigInt(1e8))
                .storeUint(0, 1 + 4 + 4 + 64 + 32 + 1 + 1)
                .endCell()
            const tx = await jettonLockup.sendMaintain(user.getSender(), msg, 3)
            expect(tx.transactions).toHaveTransaction({
                from: user.address,
                to: jettonLockup.address,
                success: false,
        ...
        it('should set data', async () => {
            const dataBefore = await jettonLockup.getContractData()
            const newData = beginCell()
                .storeInt(-1, 2)
                .storeRef(
                    beginCell()
                        .storeCoins(dataBefore.totalSupply)
                        .storeAddress(dataBefore.regulator)
                        .storeRef(beginCell())
                        .storeRef(dataBefore.walletCode)
                    .endCell()
                )
                .storeRef(
                    beginCell()
                        .storeAddress(dataBefore.authenticJwall)
                        .storeAddress(dataBefore.promiseJwall)
                    .endCell()
                )
                .storeRef(
                    beginCell()
                        .storeRef(
                            beginCell()
                                .storeUint(dataBefore.startTime-1, 32)
                                .storeUint(dataBefore.endTime-1, 32)
                                .storeUint(dataBefore.maxMonths, 4)
                            .endCell()
                        )
                        .storeRef(
                            beginCell()
                                .storeCoins(BigInt(1e9*100))
                            .endCell()
                        )
                    .endCell()
                )
            ...
            const tx = await jettonLockup.sendSetData(owner.getSender(), newData)
            expect(tx.transactions).toHaveTransaction({
                from: owner.address,
                to: jettonLockup.address,
                success: true,
            ...
            const dataAfter = await jettonLockup.getContractData()
            expect(dataAfter.startTime).toBe(dataBefore.startTime-1)
            expect(dataAfter.endTime).toBe(dataBefore.endTime-1)
            expect(dataAfter.redeemedTokens).toBe(BigInt(1e9*100))
        })

        it('shouldn`t set data, not owner', async () => {
            const dataBefore = await jettonLockup.getContractData()
            const newData = beginCell()
                .storeInt(-1, 2)
                .storeRef(
            ...
            const tx = await jettonLockup.sendSetData(user.getSender(), newData)
            expect(tx.transactions).toHaveTransaction({
                from: user.address,
                to: jettonLockup.address,
                success: false,
            ...
            const dataAfter = await jettonLockup.getContractData()
            expect(dataAfter.startTime).toBe(dataBefore.startTime)
            expect(dataAfter.endTime).toBe(dataBefore.endTime)
            expect(dataAfter.redeemedTokens).toBe(BigInt(0))
        })
    })
